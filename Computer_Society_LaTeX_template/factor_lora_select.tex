...
Measured Qwen3.5-9B runs show Affect=share and Strategy/Relation=relearn gates, near-ceiling Dir~I Acc for Scratch/Blind/SELENE, and severe Dir~II forgetting only under KO-scratch---motivating selective gates and EN-aware adapters over blind overwrite.
\end{abstract}
...
\textbf{SELENE Bank (F16)} & 87.7 & 64.2 & 86.3 & 79.4 \\
Blind +MLP (B32) & 80.9 & 62.8 & 81.2 & 75.0 \\
\textbf{SELENE Bank +MLP (F32)} & \textbf{87.0} & \textbf{64.0} & \textbf{87.4} & \textbf{79.5} \\
\bottomrule
\end{tabular}
\end{table*}
...
Table~\ref{tab:dir_ii}: KO-scratch forgets heavily ($-17.5$\,pp); Blind and SELENE retain within $\approx\!3$\,pp---EN-aware init is necessary, and gated soft-share matches Blind on retention under full KO data.
...
\midrule
After KO-scratch & 47.8 & $-17.5$ \\
After Blind share & \textbf{62.7} & $-2.7$ \\
After SELENE (v3) & 61.9 & $-3.4$ \\
\bottomrule
\end{tabular}
\end{table}
...
Table~\ref{tab:ablation} isolates stages with measured Qwen runs.
Removing EN pretraining collapses Direction~II; Blind share recovers Dir~I without gates; freeze-LoRA SELENE~(v4) protects EN weights but trades away Dir~I A/R; dual-merge SELENE~(v5) is the modular candidate for joint Dir~I/II (filled when the run completes).

\begin{table}[!t]
\caption{Stage-wise ablation (Qwen3.5-9B). Dir~I Avg $=$ mean(A,S,R).}
\label{tab:ablation}
\centering
\resizebox{\columnwidth}{!}{%
\begin{tabular}{@{}clccc@{}}
...
\midrule
A0 & SELENE soft-share v3 & 80.2 & $-3.4$ & reference \\
A1 & KO-scratch (no EN) & 78.8 & $-17.5$ & EN pretrain \\
A2 & Blind share (no gates) & 79.2 & $-2.7$ & factor gates \\
A4 & EN before KO & --- & $0.0$ & KO adapt \\
A7 & SELENE freeze-LoRA v4 & (pending Blind) & (pending) & share vs adapt \\
A8 & SELENE dual-merge v5 & (pending) & (pending) & modular LoRA \\
\bottomrule
\end{tabular}%
}
\end{table}
...
\caption{Stage-wise ablation overview.}
\label{fig:ablation}
\end{figure}
...
\subsection{Claims}
\begin{itemize}
\item \textbf{Factor pattern:} Stage~2 assigns Affect=\textit{share} and Strategy/Relation=\textit{relearn} (culture=\textit{suppress}), aligning with ESC's fragile strategy axis~\cite{liu2021esc}.
\item \textbf{Dir~I:} On full AI~Hub data, SELENE matches Blind/Scratch Acc (ceiling); gains are small (esp.\ R) rather than dominating Acc.
\item \textbf{Dir~II:} SELENE and Blind both avoid the catastrophic forgetting of KO-scratch ($-17.5$\,pp); EN init is necessary~\cite{wang2022lfmlf}.
\item \textbf{Trade-off:} Freeze-LoRA SELENE protects EN weights but loses Dir~I A/R; dual-merge SELENE targets both axes without scalar-only gating.
\end{itemize}
...
We do not claim SOTA Acc over MAD-X/AdaMergeX/FLARE without matched reimplementation; our primary baselines are Scratch and Blind share under identical data and compute.
...
\textbf{empathy does not transfer as a single skill}---gates diagnose Affect as shareable and Strategy/Relation as relearn---and EN-aware adapters prevent the English forgetting of KO-scratch~\cite{davis1983empathy,liu2021esc,lee2025koed,wang2022lfmlf}.
On full Korean supervision, Dir~I Acc is near ceiling for Scratch/Blind/SELENE; the contribution is the factor-gated protocol, dual evaluation (Dir~I/II), and modular adapters that make the share/relearn trade-off explicit.
KoED human cultural-appropriateness scoring remains future work.
